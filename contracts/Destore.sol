...
contract DeStore {
    ...
    function addApplication(
        ...
        string memory _icon,
        string memory _apkFile,
        string memory _iosFile,
        string[] memory _images
    ) public {
        require(
            keccak256(abi.encodePacked(_name)) !=
                keccak256(abi.encodePacked("")),
            "Cannot have empty app name"
        );
        require(
            keccak256(abi.encodePacked(_tagLine)) !=
                keccak256(abi.encodePacked("")),
            "Cannot have empty app tagline"
        );
        require(
            keccak256(abi.encodePacked(_description)) !=
                keccak256(abi.encodePacked("")),
            "Cannot have empty app description"
        );
        require(
            keccak256(abi.encodePacked(_icon)) !=
                keccak256(abi.encodePacked("")),
            "Cannot have empty app icon"
        );
        bytes32 _appId = keccak256(
            abi.encodePacked(_name, _tagLine, _description, _icon)
        );
        require(
            keccak256(abi.encodePacked(_apkFile)) !=
                keccak256(abi.encodePacked("")) ||
                keccak256(abi.encodePacked(_iosFile)) !=
                keccak256(abi.encodePacked("")),
            "Cannot have empty apk or ios file"
        );
        require(
            _images.length > 0 &&
                keccak256(abi.encodePacked(_images[0])) !=
                keccak256(abi.encodePacked("")),
            "Cannot have empty image list"
        );
        App memory _newApp = idToApp[_appId];
        require(
            keccak256(abi.encodePacked(_newApp.appId)) !=
                keccak256(abi.encodePacked(_appId)),
            "App id already available"
        );
